...
class ThemeZee_Icon_Block_License_Settings {
	...
	static function render_license_key_setting() {
		...
		if ( 'valid' === $license_status && ! empty( $license_key ) ) {
			...
			$html .= '<br/><span class="description">' . esc_html__( 'Active. You are receiving updates.', 'themezee-icon-block' ) . '</span>';
		} else {
			...
			$html .= '<br/><span class="description">' . esc_html__( 'Inactive. Please enter your license key.', 'themezee-icon-block' ) . '</span>';
		}

		echo $html;
		wp_nonce_field( 'themezee_icon_block_license_nonce', 'themezee_icon_block_license_nonce' );
	}
	...
	static function activate_license() {
		...
		if ( ! isset( $_POST['themezee_icon_block_activate_license'] ) ) {
			return;
		}

		// Run a quick security check.
		if ( ! check_admin_referer( 'themezee_icon_block_license_nonce', 'themezee_icon_block_license_nonce' ) ) {
			return;
		}

		// Get License key.
		$license_key = ! empty( $_POST['themezee_blocks_settings']['icon_block_license_key'] ) ? sanitize_text_field( $_POST['themezee_blocks_settings']['icon_block_license_key'] ) : false;
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							__( 'Your license key expired on %s.', 'themezee-icon-block' ),
					...
					case 'disabled':
					case 'revoked':
						$message = __( 'Your license key has been disabled.', 'themezee-icon-block' );
						break;

					case 'missing':
						$message = __( 'Invalid license.', 'themezee-icon-block' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = __( 'Your license is not active for this URL.', 'themezee-icon-block' );
						break;

					case 'item_name_mismatch':
						/* translators: the plugin name */
						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'themezee-icon-block' ), THEMEZEE_ICON_BLOCK_NAME );
						break;

					case 'no_activations_left':
						$message = __( 'Your license key has reached its activation limit.', 'themezee-icon-block' );
						break;
		...
		if ( 'valid' === $license_data->license ) {
			$options['icon_block_license_key'] = $license_key;
		}
		$options['icon_block_license_status'] = $license_data->license;
	...
	static function deactivate_license() {
		...
		// Listen for our activate button to be clicked.
		if ( ! isset( $_POST['themezee_icon_block_deactivate_license'] ) ) {
			return;
		}
		...
		$options['icon_block_license_key']    = '';
		$options['icon_block_license_status'] = 'inactive';
		update_option( 'themezee_blocks_settings', $options );
	...
	static function license_activated_notice() {
		if ( isset( $_GET['icon_block_activation'] ) && ! empty( $_GET['message'] ) ) {

			switch ( $_GET['icon_block_activation'] ) {
	...
	static function plugin_page_notice() {
		...
		if ( 'valid' !== $options['icon_block_license_status'] && in_array( $pagenow, array( 'index.php', 'update-core.php', 'plugins.php' ) ) && ! isset( $_GET['page'] ) && current_user_can( 'manage_options' ) ) :
			?>

			<div class="notice notice-info">
				<p>
					<?php
					printf( __( 'Please enter your license key for the %1$s in order to receive updates and support. <a href="%2$s">Enter License Key</a>', 'themezee-icon-block' ),
						'ThemeZee Icon Block',
